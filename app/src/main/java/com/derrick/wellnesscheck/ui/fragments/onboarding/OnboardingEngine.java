...
package com.derrick.wellnesscheck.ui.fragments.onboarding;
...
public class OnboardingEngine implements PaperOnboardingEngineDefaults {
    ...
    public OnboardingEngine(View rootLayout, ArrayList<PaperOnboardingPage> contentElements, Context appContext) {
        if (contentElements != null && !contentElements.isEmpty()) {
            ...
            this.mRootLayout.getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
                public void onGlobalLayout() {
                    ...
                    OnboardingEngine.this.mPagerElementActiveSize = OnboardingEngine.this.mPagerIconsContainer.getHeight();
                    OnboardingEngine.this.mPagerElementNormalSize = Math.min(OnboardingEngine.this.mPagerIconsContainer.getChildAt(0).getHeight(), OnboardingEngine.this.mPagerIconsContainer.getChildAt(OnboardingEngine.this.mPagerIconsContainer.getChildCount() - 1).getHeight());
                    MarginLayoutParams layoutParams = (MarginLayoutParams)OnboardingEngine.this.mPagerIconsContainer.getChildAt(0).getLayoutParams();
                    OnboardingEngine.this.mPagerElementLeftMargin = layoutParams.leftMargin;
                    OnboardingEngine.this.mPagerElementRightMargin = layoutParams.rightMargin;
                    OnboardingEngine.this.mPagerIconsContainer.setX((float)OnboardingEngine.this.calculateNewPagerPosition(0));
                    OnboardingEngine.this.mContentCenteredContainer.setY((float)((OnboardingEngine.this.mContentRootLayout.getHeight() - OnboardingEngine.this.mContentCenteredContainer.getHeight()) / 2));
                }
            });
        } else {
    ...
    protected AnimatorSet createBGAnimatorSet(final int color) {
        ...
        bgAnimSet.addListener(new AnimatorEndListener() {
            public void onAnimationEnd(Animator animation) {
                OnboardingEngine.this.mRootLayout.setBackgroundColor(color);
                bgColorView.setVisibility(View.GONE);
                OnboardingEngine.this.mBackgroundContainer.removeView(bgColorView);
            }
        });
        return bgAnimSet;
    }
    ...
    private AnimatorSet createContentTextShowAnimation(final View currentContentText, View newContentText) {
        ...
        currentContentMoveUp.addListener(new AnimatorEndListener() {
            public void onAnimationEnd(Animator animation) {
                OnboardingEngine.this.mContentTextContainer.removeView(currentContentText);
            }
        });
    ...
    protected AnimatorSet createContentIconShowAnimation(final View currentContentIcon, View newContentIcon) {
        ...
        currentContentMoveUp.addListener(new AnimatorEndListener() {
            public void onAnimationEnd(Animator animation) {
                OnboardingEngine.this.mContentIconContainer.removeView(currentContentIcon);
            }
        });
    ...
    protected Animator createContentCenteringVerticalAnimation(View newContentText, View newContentIcon) {
        newContentText.measure(MeasureSpec.makeMeasureSpec(this.mContentCenteredContainer.getWidth(), MeasureSpec.AT_MOST), -2);
