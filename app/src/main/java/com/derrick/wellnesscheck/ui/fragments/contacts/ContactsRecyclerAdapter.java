...
public class ContactsRecyclerAdapter extends RecyclerView.Adapter<ContactsRecyclerAdapter.ViewHolder> {
    ...
    private void requestNewRiskLvl(){
        final SmsReceiver smsReceiver = new SmsReceiver();

        AlertDialog alertDialog = new AlertDialog.Builder(activity, R.style.AppTheme_Dialog_Alert)
        ...
        StringBuilder sb = new StringBuilder(activity.getString(R.string.new_risk_lvl_request));
        sb.append("\n\nI'm currently rated as " + mData.get(selectedPos).getRiskLvl() + " risk by you.");
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId()) {
                case R.id.context_menu_item_delete:
                    ...
                    return true;
                case R.id.context_menu_request_new_risk_lvl:
                    if(Prefs.confirmAskRiskLvl()) {
                        View dialogView = LayoutInflater.from(getContext()).inflate(R.layout.confirmation_dialog, null);
                        TextView moreInfo = dialogView.findViewById(R.id.confirm_more_info);
                        moreInfo.setText(R.string.confirm_new_risk_msg);
                        new AlertDialog.Builder(getContext(), R.style.AppTheme_Dialog_Alert)
                                .setView(dialogView)
                                .setMessage("Are you sure you want to ask " + mData.get(selectedPos).name + " for a new risk level?")
                                .setPositiveButton("Yes", (dialog, which) -> {
                                    if (((CheckBox) dialogView.findViewById(R.id.never_ask_again)).isChecked())
                                        Prefs.confirmAskRiskLvl(false);
                                    requestNewRiskLvl();
                                })
                                .setNegativeButton(R.string.cancel, null)
                                .setCancelable(false)
                                .show();
                    }else requestNewRiskLvl();
                    return true;
                default:
                    return false;
            }
